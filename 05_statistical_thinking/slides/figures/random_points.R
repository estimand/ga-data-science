...
random.points <- function(n.points=250, n.squares=1) {
    set.seed(1)
	limits <- seq(0, 1, length.out=n.squares+1)
	n <- n.points / n.squares**2
	plot(NA, type="n", xlim=range(limits), ylim=range(limits), xlab="", ylab="")
